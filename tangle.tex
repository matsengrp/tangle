...
It is more helpful to give labels to the leaves of these tree shapes in order to discuss bijections between them.
We will call these \emph{standardized} phylogenetic trees: a choice of representative from the set of pre-images $\pi_\shape(s)$: one (leaf-labeled) phylogenetic tree $\{t_{(i)}\}_{i=1,\ldots,k}$ for each tree shape $s_i$.
For any phylogenetic tree $t$ the \emph{standard permutation} is the permutation $\sigma \in \fS_n$ mapping the leaf labels of its standardized tree $s_i$ to those of $t$.
...
\end{definition}
Given a standardization, this definition is the same as that in Definition~\ref{def:abstractTanglegram}, where the bijection $\phi$ in the first definition results from a $\pairing$ by applying the $\pairing$ action to the standardized leaf set.
To avoid confusion, we emphasize that the action of the symmetric group is by relabeling items, not by moving them about in a planar representation.
