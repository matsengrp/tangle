...
We will do so by taking \emph{standardized} tree topologies and combining them with various cosets as per Observation~\ref{obs:cosetMap}.
These standardized topologies are simply a choice of one labeled tree topology $\{s_i\}_{i=1,\ldots,k}$ for each isomorphism class of phylogenetic trees on $n$ leaves under graph-theoretic isomorphism (as unlabeled tree topologies).
Any tree $t$ has a representative $s_i$ of its equivalence class, and there is a corresponding $\sigma \in \fS_n$ that maps the leaf labels of $s_i$ to those of $t$.
...
The group element $\sigma_1^{-1} \, \sigma_2$ maps the leaves of $s_{(1)}$ to the leaves of $s_{(2)}$ that they correspond to in the $(t_1, t_2)$ tanglegram.
...
Thus the triple $(s_{(1)}, s_{(2)}, \aut(s_{(1)}) \sigma_1^{-1} \, \sigma_2 \aut(s_{(1)}))$ defines the tanglegram formed by the two phylogenetic trees $t_1$ and $t_2$.
