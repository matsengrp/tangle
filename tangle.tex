...
The concept of a pair of phylogenetic trees with a relative leaf labeling can be formalized as the graph-theoretic notion of a \emph{tanglegram}.
...
In order to be able to uniquely describe elements of the symmetric group, we will use the convention that the first tree in planar tanglegrams is labeled clockwise, and the second tree is labeled counter-clockwise, so that they end up looking like so
...
[Or do we always want them in the vertical orientation?]
...
In fact, we will show is a one-to-one correspondence between classes of planar tanglegrams $(t_1, t_2, \pairing)$ that have the same tanglegram, and ordered triples $(t_1, t_2, \aut(t_1) \pairing \aut(t_2))$ composed of two phylogenetic trees and a double coset.
[gh-3: Say something about utanglegrams?]

\begin{observation}
There is a well-defined injective map from $n$-tanglegrams to ordered triples $(t_1, t_2, C)$ where $C$ is a double coset of $\fS_n$ defined by taking an arbitrary planar embedding $(t_1, t_2, \pairing)$ of the tanglegram and mapping
\[
(t_1, t_2, \pairing) \mapsto (t_1, t_2, \aut(t_1) \pairing \aut(t_2)).
\]
\end{observation}
...
It is injective because any if any pair of planar tanglegrams $(t_1, t_2, \pairing)$ and $(t'_1, t'_2, \pairing')$ map to the same element, it means that the trees are (graph theoretically) isomorphic, defining $\alpha_i$ mapping $t_i$ to $t'_i$; then $\pairing = \alpha_1 \pairing' \alpha_2^{-1}$ and thus that they have the same double coset.

This map is thus a bijection onto its image, and so we will refer, e.g., to the coset of a tanglegram.

\LB{I'm stopping here for now. Will make my way through the rest of the stuff later.}

\begin{lemma}
There is a bijection between $n$-tanglegrams $x = (t_1, t_2, \pairing)$, $\aut(x)$ is the exactly group of graph automorphisms of the corresponding tanglegraph $g_x$.
\end{lemma}
\begin{proof}
