...
The labelings of an $n$-tanglegram will follow a similar pattern.
Assume a fixed planar embedding of two tree shapes $s_1$ and $s_2$ with $n$ leaves, and assume that the leaves are labeled arbitrarily to give planar phylogenetic trees $p_1$ and $p_2$.
We will define a bijective mapping of the direct product $\fS_n \times \fS_n$ to planar labeled tanglegrams in which
\[
(\sigma, \pairing) \mapsto (p_1.\sigma, p_2, \pairing)
\]
and will thus consider the enumeration problem on $\fS_n \times \fS_n$.
We are interested in the labelings of a given (non-planar) tanglegram, and assume that $(\sigma, \tau)$ is an exemplar of such a tanglegram.
Define a right action of $\aut(p_1) \times \aut(p_2)$ on $\fS_n \times \fS_n$ by
\[
(\sigma, \pairing). (\tau_1, \tau_2) := (\sigma \tau_1, \pairing \tau_2).
% (\sigma, \pairing). (\tau_1, \tau_2) := (\sigma \tau_1, \tau_1^{-1} \pairing \tau_2).
\]
This action corresponds to relabeling $p_1$ by $\tau_1$ and $p_2$ by $\tau_2$, and results in the same tanglegram by the definition of $\aut$.
Now consider equivalence classes of labeled (non-planar) tanglegrams.
We still have this action, and what we end up with is the orbit of cosets $A_1 \tau_1$ on the right.
...
For stanglegrams with $t = t_1 = t_2$ the number can be smaller due the additional symmetry of exchanging $\ell_1$ and $\ell_2$.
We can express this symmetry by its impact on the labeling of $t_1$, which gets the labeling $\ell_1.\pairing$.
Indeed, then a complete enumeration will only have one of $(t, t.\mu, \id)$ and $(t, t.\mu^{-1}, \id)$.
These will also be considered to be the same in a list of unordered pairs of phylogenetic trees.
