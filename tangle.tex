...
Labeled planar $n$-tanglegrams with a given pair of tree shapes (i.e.\ elements of $\pi_\shape^{-1}(s_1, s_2)$ for some $s_1$ and $s_2$) are in one to one correspondence with pairs of permutations.
...
Because flipping the tree before labeling doesn't do anything (recall that tree shapes are identical), the collection of edge colored tanglegrams is equivalent to the right cosets $\langle \aut(t_1) \times \aut(t_2) \times C_2 \rangle \, (\sigma_1, \sigma_2)$ for $\sigma_1, \sigma_2 \in \fS_n$.
...
\[
\langle \aut(t_1) \times \aut(t_2) \times C_2 \rangle \, (\sigma_1, \sigma_2) \, \langle {\id} \times {\id} \times C_2 \rangle$ for $\sigma_1, \sigma_2 \in \fS_n.
\]


\section{Enumeration}
\EM{plow this paragraph into the above?}
